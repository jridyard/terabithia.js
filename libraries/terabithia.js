...
var handleCommandFromContent = async (command, body) => {
    switch (command) {
        default:
            console.log(yellowSolidBg, `${framework}: Handled unexpected command via handleCommandFromContent: `, command);
            return;
    };
};
...
async function getElementsReactProps(element) {
    if (!element) {
        console.log('\x1b[31m%s\x1b[0m', `${framework}: getElementsReactProps requires an element. The one passed was not found`);
        return null;
    };
    ...
    const propsRequest = await sendToPage({command: "getReactProps", selector: `[data-terabithia-target="${targetId}"]`});
...
async function triggerElementsReactEvent(element, event) {
    if (!element) {
        console.log('\x1b[31m%s\x1b[0m', `${framework}: triggerElementsReactEvent requires an element. The one passed was not found`);
        return null;
    };
...
async function sendToCounterpartContext(json, bridgeId=COUNTERPART_BRIDGE_ID) {
    return new Promise((resolve) => {
        if (!json?.command && bridgeId === COUNTERPART_BRIDGE_ID) return console.log(regSolidBg, `${framework}: sendToContent requires a COMMAND property passed via JSON.`);
    ...
    var getChromeExtensionStorage = async (key) => {
        return new Promise((resolve) => {
            if (localStorageKeysAccessibleForPage && !localStorageKeysAccessibleForPage.includes(key)) {
                console.log(redText, `${framework}: getChromeExtensionStorage received a key that was not approved for PAGE access: `, key);
                return resolve(null);
            };
        ...
        switch (command) {
            case "triggerReactEvent":
                ...
                if (!eventHandler) {
                    ...
                    else if (!eventHandler) errorMsg = `No <${body.event}> handler found on element with selector ${body.selector}.`;
                    ...
                    try {
                        ...
                        console.log(redText, `${framework}: Error triggering ${body.event} on element with selector ${body.selector}: `, err);
                        response = {
                            success: false,
            ...
            case "getReactProps":
                ...
                if (element) {
                    ...
                    var reactPropsNoCallbacks = Object.keys(reactProps).filter(key => typeof reactProps[key] !== 'function').reduce((obj, key) => {
